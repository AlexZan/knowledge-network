...
The moment I send that message, something happens. The system recognizes I've accepted the answer. I see:

```
[Conclusion extracted: "401 API error was caused by expired authentication token. Solution: refresh the token before making API calls."]
[Tokens: 1,247 raw → 68 compacted | Savings: 95%]
```

The verbose back-and-forth about the 401 error has been compressed into a single sentence. I can see exactly how many tokens were saved.
...
Again, the system detects my acceptance:

```
[Conclusion extracted: "Slow database queries caused by N+1 problem - fetching related records in loop instead of using joins."]
...
The AI responds referencing both conclusions - the token refresh was a quick fix, while the N+1 required refactoring queries. It remembers both issues without me re-explaining them, and without loading the full conversation history.
...
The AI immediately recalls: the 401 token expiration and the N+1 database problem. My conclusions persisted. I'm back in the same conversation, picking up where I left off.

I continue debugging a new issue, and the cycle continues - questions, answers, conclusions extracted, tokens saved.
...
3. When I agreed/thanked, conclusions appeared automatically
4. Token savings were shown after each conclusion
5. Later questions could reference earlier conclusions
6. Exiting and restarting continued the same conversation
7. The AI knew things I'd concluded earlier without me repeating them

## What I Didn't Have To Do

- No commands to memorize
- No manual "save" or "conclude" actions
