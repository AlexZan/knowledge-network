...
**The 5x threshold is deliberately conservative.** A factual claim must have five times the support of its competitor before automatic resolution. This means a 4:1 advantage — which would be overwhelming in most contexts — still requires human review. The threshold was chosen to prevent premature action at the cost of leaving resolvable conflicts for manual review. The threshold's value is in ensuring abstention when evidence is ambiguous — resolution correctness is bounded by the quality of edges the LLM constructed.
...
**Conservative abstention protects against construction noise.** The 5x threshold ensures that construction artifacts — which accounted for 55% of post-resolution conflicts (Section 4.2) — are never auto-resolved. Auto-resolution correctness depends on the quality of edges the LLM built; the topology's contribution is knowing when *not* to act. A spot-check of 15 auto-resolutions confirmed that the topology consistently picks the more-supported side, but also revealed one case where a false `contradicts` edge (scope mismatch between sources) led to an incorrect auto-resolution — the construction error, not the topology, was at fault.
...
This is not a claim that topological support equals truth. A well-argued falsehood would accumulate topological support just as readily as a well-argued truth. The system measures *coherence of supporting structure*, not *correspondence to reality*. What the topology provides is **accessible truth** — the best truth available from the evidence the graph contains. A 27:2 support ratio for an established theory over an unpublished alternative is not wrong — it accurately reflects the current state of evidence. Ingest experimental results that validate the alternative, and the ratio shifts.

This limitation is shared by every epistemological framework, including science. Scientific replication measures coherence of experimental results — it cannot prove correspondence to ultimate reality, only convergence of independent evidence. Topological truth-finding operates at the same level: it identifies claims that are well-supported by independent structural evidence, and distinguishes them from claims that are not. The graph's truth is bounded by its inputs and revisable as those inputs grow.
