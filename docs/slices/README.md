...
9. ~~**Topology-based support weight**~~ ✅ — Replace binary edge weight (1.0/0.5) in `compute_all_confidences()` with computed weight from embedding dissimilarity + source independence. `_compute_edge_weight()` in `confidence.py` uses cosine similarity (linear ramp: 0.95→0.70 maps to 0→1) × source factor (0.2 same conversation, 0.5 same author, 1.0 different source). Results on v4 KG: fact-043 support -32%, contested nodes 64→25 (false paraphrase contradictions eliminated), 165 inflated medium→low. Backward compatible (falls back to legacy without embeddings). See `docs/research/support-weight-from-topology.md` for design, `docs/research/v4-linking-findings.md` for validation.
...
- **Unified ingestion pipeline** — `ingest_pipeline()` and `ingest_chatgpt_export()` duplicate stages 4-8 (auto-link, link, embed, cluster, conflict report). Refactor into a shared `_post_write_pipeline(node_ids, session_dir, ...)` called by both entry points. Parse/extract/write differs per format (documents vs ChatGPT conversations), but everything after write is identical.
